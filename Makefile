.PHONY: mk-docker-image noos-build
mk-docker-image:
	cd .buildutil && $(MAKE) setup-build
noos-build:
	sudo $(MAKE) -f .buildutil/Makefile build-checkpoint
emulate:
	qemu-system-x86_64 -cdrom dist/x86_64/kernel.iso
clean:
	rm -rf dist build targets/x86_64/iso/boot/kernel.bin
run: all emulate

all: clean mk-docker-image noos-build
